...
import com.handmark.pulltorefresh.library.internal.FlipSpecialLoadingLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		if (!isPullToRefreshEnabled() || !mIsDrag) {
			return false;
		}
				...
				if (isReadyForPull()) {
					...
					if (absDiff > mTouchSlop && (!mFilterTouchEvents || absDiff > Math.abs(oppositeDiff))) {
						if (mMode.showHeaderLoadingLayout() && diff >= 1f && isReadyForPullStart()) {
							...
							Log.i(LOG_TAG, "onInterceptTouchEvent mIsBeingDragged true!");
							if (mMode == Mode.BOTH) {
	...
	public final boolean onTouchEvent(MotionEvent event) {
       //if(mIsDrag){
		...
		switch (event.getAction()) {
			case MotionEvent.ACTION_MOVE: {
				Log.i(LOG_TAG,"onTouchEvent ACTION_MOVE: mIsBeingDragged:"+mIsBeingDragged);
				if (mIsBeingDragged) {
	...
	protected final void setHeaderScroll(int value) {
			...
			if (DEBUG) {
				Log.d(LOG_TAG, "setHeaderScroll!!!: " + value+"|mMode:"+(mMode==Mode.DISABLED));
			}
			
			
			if(!mIsBeingDragged || !mIsDrag){
				//mIsBeingDragged=true;
				//scrollTo(0, 0);
				return;
			}
	...
	protected void updateUIForMode() {
		 Log.d(LOG_TAG,"come updateUIForMode");
		// We need to use the correct LayoutParam values, based on scroll
		// direction
		final LinearLayout.LayoutParams lp = getLoadingLayoutLayoutParams();
		 Log.d(LOG_TAG,"come lp");
		// Remove Header, and then add Header Loading View again if needed
		if (this == mHeaderLayout.getParent()) {
			removeView(mHeaderLayout);
			 Log.d(LOG_TAG,"come removeView header");
		}
		if (mMode.showHeaderLoadingLayout() ) {
        ...
        Log.d(LOG_TAG,"hide Loading Views");
		// Hide Loading Views
		refreshLoadingViewsSize();
	...
	private void pullEvent() {
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			Log.i(LOG_TAG,"newScrollValue != 0 && !isRefreshing() |"+newScrollValue+"|publl state:"+mState+"|itemDimension:"+itemDimension+"|newscrollval:"+ Math.abs(newScrollValue));
	...
	private final void smoothScrollTo(int newScrollValue, long duration, long delayMillis,
        ...
        Log.i(LOG_TAG,"oldsv:"+oldScrollValue+"|newsv:"+newScrollValue);
	...
	public static enum AnimationStyle {
		...
		LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
			switch (this) {
				...
				case S_FLIP:
					return new FlipSpecialLoadingLayout(context, mode, scrollDirection, attrs);
			}
		}
	}
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			if (mStartTime == -1) {
				...
				Log.i(LOG_TAG,"mCurrentY£º"+mCurrentY);
